...
function leagueKey(league) {
  return (league || 'Outra').trim().toLowerCase();
}

/** Times elegíveis ao sorteio: respeita bans; modo liga usa sempre o masterDB; modo 80+ restringe OVR. */
function buildMatchmakingPool() {
  ...
  const eliteOnly = document.getElementById('elite80Toggle')?.checked;
  const noBanned = (arr) => arr.filter(t => !bannedTeams.includes(t.n));

  let pool;
  if (leagueMode && selectedLeague !== 'all') {
    const key = leagueKey(selectedLeague);
    pool = noBanned(masterDB.filter(t => leagueKey(t.league) === key));
  } else {
    pool = noBanned(teamsDB);
  }
  if (eliteOnly) {
    pool = pool.filter(t => getOVR(t) >= 80);
  }
  return pool;
}
...
async function boot() {
  ...
  window.addEventListener('storage', async (e) => {
    if (e.key !== 'fc26_custom_db') return;
    masterDB = await fetchTeamsData();
    applyLeagueFilter(true);
    showToast('Lista de times atualizada (admin)');
  });
...
window.applyLeagueFilter = function(silent) {
  const val = document.getElementById('leagueFilter').value;
  if (val === 'all') {
    teamsDB = [...masterDB];
  } else {
    const key = leagueKey(val);
    teamsDB = masterDB.filter(t => leagueKey(t.league) === key);
  }
  updateSummary();
  if (!silent) showToast(`Filtro: ${teamsDB.length} times disponíveis`);
  updateLeagueModeRow();
};
...
window.startDraft = function() {
  ...
  const draftPool = buildMatchmakingPool();
  if (!tournState.active && draftPool.length < 2) {
    showToast('Poucos times para sortear (filtro/liga/80+/bans). Ajuste as opções em Config.');
    return;
  }
  ...
  const iv=setInterval(()=>{
    const r1=draftPool[Math.floor(Math.random()*draftPool.length)];
    const r2=draftPool[Math.floor(Math.random()*draftPool.length)];
...
function finalize() {
  ...
  const leagueMode = document.getElementById('leagueModeToggle')?.checked;
  const basePool = buildMatchmakingPool();
  if (basePool.length < 2) {
    showToast('Liga ou filtro sem times suficientes (verifique bans e modo 80+).');
    isDrafting = false;
    document.getElementById('btnSortear').disabled = false;
    document.getElementById('tab-arena').classList.remove('shuffling');
    return;
